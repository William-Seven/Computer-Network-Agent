...
- **👁️ 多模态视觉感知**：自动从 PDF 中提取图片并进行向量化，Agent 能够结合图片内 容（如网络拓扑图、实验结果截图）回答问题。同时支持**前端直接上传图片**，用户可发送实验报错截图或网络拓扑图，系统将通过 Vision 大模型直接解析上传图片并结合当前上下文给出解答。
...
### 2. 端口规划
为避免前后端分离导致的跨域和跨主机请求问题，本项目将前端静态资源集成到了 FastAPI 后端中统一托管。

| 服务     | 端口     | 说明                                    |
| -------- | -------- | --------------------------------------- |
| 后端 API | **8111** | 提供 `/auth/*` 及 `/chat/*` 核心接口 |
| 前端 UI  | **8111** | 静态页面服务整合至 FastAPI (通过 `/` 访问) |
